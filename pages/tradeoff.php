<?PHP error_reporting(E_ALL || ~E_WARNING);
 if(!defined('INITIALIZED')) exit;
    $page = 0;
		foreach($SQL->query('SELECT * FROM `auction_system` WHERE item_name LIKE "%'.$pesquisar.'%" ORDER BY `item_name` DESC') as $opa)
$auctions = $SQL->query('SELECT `auction_system`.`player_id`, `auction_system`.`id`, `auction_system`.`item_name`, `auction_system`.`item_id`, `auction_system`.`count`, `auction_system`.`value`, `auction_system`.`date`, `players`.`name` FROM `auction_system`, `players` WHERE `players`.`id` = `auction_system`.`player_id` ORDER BY `auction_system`.`id` DESC')->fetchAll();
$players = 0;
        ...
        if($action == "item")
        {
            if($pesquisar)
            {
                ...
                <tr>
                    ...
                    <td><center>!tradeoff buy, '.$info['id'].'</center>
                </tr>';
            }
        }
                                    ...
                                    <tr>
                                        <td>
                                            <div class="TableShadowContainerRightTop">
                                                <div class="TableShadowRightTop" style="background-image:url(layouts/tibiacom/images/global/content/table-shadow-rm.gif);"></div>
                                            </div>
                                                                ...
                                                                <tbody>
                                                                    <tr>
                                                                        <td width="40%" style="background: color: white;">
                                                                            ...
                                                                            <br><small>example: !tradeoff buy, 1943<br>Esse comando compra uma oferta e vai direto pro Inbox no DP.</small></center>
                                                                        </td>
                                                                        <td width="40%" style="background: color: white;">
    ...
    <div class="TableContainer">
        <div class="CaptionContainer">
            <div class="CaptionInnerContainer">
                ...
                <span class="CaptionBorderTop" style="background-image:url(layouts/tibiacom/images/global/content/table-headline-border.gif);"></span>
                <span class="CaptionVerticalLeft" style="background-image:url(layouts/tibiacom/images/global/content/box-frame-vertical.gif);"></span>
                <div class="Text"><font color="white">Trade OFF</font></a>
